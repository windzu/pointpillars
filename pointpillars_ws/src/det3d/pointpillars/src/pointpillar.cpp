...
TRT::TRT(std::string modelFile, cudaStream_t stream) : stream_(stream) {
  ...
  if (!trtCache.is_open()) {
    ...
    if (!parser->parseFromFile(
      ...
      std::cerr << ": failed to parse onnx model file, please check the onnx "
                   "version and trt support op!"
                << std::endl;
      exit(-1);
    }
...
PointPillar::PointPillar(std::string modelFile, cudaStream_t stream)
    : stream_(stream) {
  checkCudaErrors(cudaEventCreate(&start_));
  checkCudaErrors(cudaEventCreate(&stop_));
  ...
  checkCudaErrors(
      cudaMallocManaged((void **)&features_input_, features_input_size_));
  checkCudaErrors(
      cudaMallocManaged((void **)&params_input_, sizeof(unsigned int)));

  checkCudaErrors(
      cudaMemsetAsync(features_input_, 0, features_input_size_, stream_));
  checkCudaErrors(
      cudaMemsetAsync(params_input_, 0, sizeof(unsigned int), stream_));

  // output of TRT -- input of post-process
  cls_size_ = params_.feature_x_size * params_.feature_y_size *
              params_.num_classes * params_.num_anchors * sizeof(float);
  box_size_ = params_.feature_x_size * params_.feature_y_size *
              params_.num_box_values * params_.num_anchors * sizeof(float);
  dir_cls_size_ = params_.feature_x_size * params_.feature_y_size *
                  params_.num_dir_bins * params_.num_anchors * sizeof(float);
  ...
  bndbox_size_ = (params_.feature_x_size * params_.feature_y_size *
                      params_.num_anchors * 9 +
                  1) *
                 sizeof(float);
...
int PointPillar::doinfer(void *points_data, unsigned int points_size,
  ...
  pre_->generateFeatures(voxel_features_, voxel_num_, voxel_idxs_,
                         params_input_, features_input_);
  ...
  void *buffers[] = {features_input_, voxel_idxs_, params_input_,
                     cls_output_,     box_output_, dir_cls_output_};
  ...
  for (int i = 0; i < num_obj; i++) {
    auto Bb = Bndbox(output[i * 9], output[i * 9 + 1], output[i * 9 + 2],
  ...
  // <<std::endl; std::cout<<"TIME: generateFeatures: "<< generateFeaturesTime
  // <<" ms." <<std::endl; std::cout<<"TIME: doinfer: "<< doinferTime <<" ms."
  // <<std::endl; std::cout<<"TIME: doPostprocessCuda: "<<
  // doPostprocessCudaTime <<" ms." <<std::endl;
#endif
  return 0;
}
